...
////
////			sprintf(buffer_temp, "/bin/dd if=%s bs=32M  iflag=skip_bytes,count_bytes skip=%ld count=8000000000 | ssh m%d 'cat > temp'", argv[1], skip, (i + 2));
////			printf("%s\n", buffer_temp);
////			if (system(buffer_temp) == -1)
////				perror("System");
//
//			sprintf(buffer_temp, "scp sort_client.c m%d:", (i + 2));
//			printf("%s\n", buffer_temp);
//			if (system(buffer_temp) == -1)
//				perror("System");
//
//			sprintf(buffer_temp, "ssh m%d 'gcc -o sort_client sort_client.c "
//					"-march=native -Ofast -std=c99 -lm -pedantic "
//					"-pthread -fopenmp -funroll-loops'", (i + 2));
//			printf("%s\n", buffer_temp);
//			if (system(buffer_temp) == -1)
//				perror("System");
//
//			sprintf(buffer_temp, "/usr/bin/ssh m%d export REMOTEIP=%s && export REMOTEPORT=%d && ./sort_client", (i + 2), argv[2], (5555 + i));
//			sprintf(buffer_temp, "/usr/bin/ssh m%d ./sort_client %s %d", (i + 2), argv[2], (5555 + i));
//
//			printf("%s\n", buffer_temp);
...
//
//	for (int i = 0; i < MAXCONN; i++) {
//		sprintf(buffer_temp, "ssh m%d 'cat comp_and_run'", (i + 2));
//		printf("%s\n", buffer_temp);
//		if (system(buffer_temp) == -1)
//			perror("System");
//	}
