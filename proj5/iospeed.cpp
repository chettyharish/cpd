...
int testfreadwithbufsiz(char * name, int N) {
	int answer = 0;
	FILE * fd = ::fopen(name, "rb");
	setvbuf(fd, NULL, _IOFBF, 8192); // large buffer
	if (fd == NULL) {
		cerr << "problem" << endl;
		return -1;
	}
...
int main() {
	...
	for (int T = 0; T < 30; ++T) {
		...
		// fread with bufsiz buffer
		cput.reset();
		wct.reset();
		for (int x = 0; x < 10; ++x)
			tot += testfreadwithbufsiz(name, N);
		cout << "fread w bufsiz buffer\t\t" << 512 * N * 1.0 / cput.split() << " " << 512 * N * 1.0 / wct.split() << endl;
